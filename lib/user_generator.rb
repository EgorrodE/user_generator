...
$US_LETTERS = 	"qwertyuiopasdfghjkklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM"
$DIGITS = "0123456789"							 
...
def initialize_db
	...
	Dir.foreach($FILES_PATH) do |file|
		next if file == '.' or file == '..' or file.nil?
...
def file_to_db(filename)
	if filename.include? "cities" then
...
def write_insert_stats(count, table_name)
	puts("-----------------------------\n" +
			 "added #{count} lines to #{table_name} table\n" +
			 "-----------------------------\n")
end
...
def swap_near(line,chars)
	while true
			i = rand(chars.size)
			if(i != chars.size - 1 && chars[i][0] == chars[i+1][0] - 1) then
...
def generate_zip(state_id)
	...
	when "RU", "BY"
		substitute_x(find_by_id(table_name, state_id)[1])
	end
end
...
def generate_street_n_build_no
	...
	when "US"
		types = ["streets","street_sufixes"]
		return  (1+rand(1000)).to_s + " " + 
			generate_by_types_array(types, " ", "") + ", "
	when "RU", "BY"
		return generate_by_type("streets")[1] + ", " + (1 + rand(100)).to_s + ", "
	end
end
...
def rand_from_table(table_name)
	if(table_name!="") then
		...
		return find_by_id(table_name, id)
	end
	""
end
